...
function Navbar() {
  ...
  const cartDropdown = (close, isMobile = false) => (
    <div
      className={`${
        isMobile
          ? "fixed left-1/2 -translate-x-1/2 top-16 w-[90vw] max-w-sm"
          : "absolute right-0 top-full mt-2 w-80 max-w-[90vw]"
      } bg-card-bg text-text rounded-2xl shadow-2xl border border-snd-bg p-4 z-[1400]`}
    >
      <div className="flex items-center justify-between mb-3">
        <p className="font-bold text-lg">{t("cart.title", "Carrito")}</p>
        <button onClick={close} className="text-sm text-main font-semibold">
          {t("cart.close", "Cerrar")}
        </button>
      </div>
      ...
      {cartItems.length === 0 ? (
        ...
        <div className="space-y-3">
            ...
            <div key={item.id} className="flex items-start gap-3 rounded-xl bg-snd-bg/50 p-3">
              ...
              <div className="flex flex-col items-end gap-2">
                <div className="flex items-center gap-2">
                  <button onClick={() => updateQuantity(item.id, item.platform, item.quantity - 1)} className="w-7 h-7 rounded-full bg-bg text-text font-bold">−</button>
                  <span className="text-sm font-semibold">{item.quantity}</span>
                  <button onClick={() => updateQuantity(item.id, item.platform, item.quantity + 1)} className="w-7 h-7 rounded-full bg-bg text-text font-bold">+</button>
                </div>
                <button onClick={() => removeItem(item.id, item.platform)} className="text-xs text-main font-semibold">{t("cart.remove", "Quitar")}</button>
              </div>
            </div>
          ))}
          ...
          <Link to="/carrito" onClick={close} className="block w-full text-center rounded-full bg-main hover:bg-hover text-white py-2 font-semibold">
            {t("cart.viewCart", "Ver carrito")}
          </Link>
          <p className="text-xs text-text/60 text-center">{t("cart.viewCartHint", "Ahí vas a ver todos tus paquetes y la opción de comprar.")}</p>
        </div>
      )}
    </div>
  );
  ...
  const profileDropdown = () => (
    <div className="absolute right-0 top-full mt-2 w-64 max-w-[90vw] bg-card-bg text-text rounded-xl shadow-lg p-4 z-[1300] transition-colors duration-300">
      ...
      <button
        onClick={() => { navigate("/perfil"); setShowUserMenu(false); setMenuOpen(false); }}
      ...
      <button onClick={handleLogout} className="w-full mt-2 text-sm font-bold text-main hover:text-hover transition-colors text-left">
  ...
  return (
    <header className="relative flex flex-col lg:flex-row justify-between items-center w-full h-auto lg:h-20 p-4 lg:px-[4.375rem] lg:py-0 bg-bg shadow-[0_0.125rem_0.625rem_rgba(0,0,0,0.08)] mb-[1.875rem] transition-colors duration-[400ms]">
      {/* Barra superior para móvil y tablet (lg:hidden) */}
      <div className="flex lg:hidden items-center justify-between w-full gap-1">
        {/* Izquierda: Menú + Idioma + Tema */}
        <div className="flex items-center gap-1.5 sm:gap-2">
          <button
            type="button"
            className="text-[1.8rem] sm:text-[2rem] cursor-pointer text-text leading-none"
          ...
          <button
            onClick={toggleTheme}
            className="text-lg sm:text-xl text-accent hover:rotate-12 transition cursor-pointer ml-1"
        ...
        <Link to="/" className="flex items-center justify-center">
          <img
            ...
            alt="The Sims 4 Logo"
            className="w-16 sm:w-20 md:w-24 h-auto object-contain max-h-10"
          />
        </Link>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          {canAccessCart(user) && (
            <div className="relative" ref={mobileCartWrapperRef}>
              <button
                className="relative text-xl sm:text-2xl text-text"
                onClick={() => setShowCart((prev) => !prev)}
                aria-label={t("cart.title", "Carrito")}
              >
                🛒
                {cartItems.length > 0 && (
                  <span className="absolute -top-1 -right-1 bg-main text-white text-[0.6rem] font-bold rounded-full w-4 h-4 flex items-center justify-center">
                    {cartItems.length}
                  </span>
                )}
              </button>
              {showCart && cartDropdown(() => setShowCart(false), true)}
            </div>
          )}
          ...
          {user ? (
            <div className="relative" ref={mobileUserMenuRef}>
              <button
                ...
                className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-main text-white font-bold flex items-center justify-center hover:bg-hover transition-colors shadow-md text-xs sm:text-sm"
            ...
            <button
              ...
              className="text-xs sm:text-sm font-bold text-main hover:text-hover transition-colors"
            >
      ...
      {/* Logo exclusivo para Desktop (lg+) */}
      <div className="hidden lg:flex items-center justify-start lg:mr-8">
        <Link to="/">
          <img
            src="https://res.cloudinary.com/w1jl4sa5/image/upload/v1784825556/Logo_of_The_Sims_4.svg_jagzsl.webp"
            alt="The Sims 4 Logo"
            className="w-[7.5rem] h-auto max-h-16 object-contain"
          />
        </Link>
      </div>

      {/* Menú desplegable y navegación principal */}
      <div className="contents" ref={mobileNavWrapperRef}>
        <nav
          className={`absolute lg:static top-16 lg:top-auto left-0 w-full lg:w-auto bg-snd-bg lg:bg-transparent shadow-[0_0.375rem_1.125rem_rgba(0,0,0,0.25)] lg:shadow-none overflow-hidden lg:overflow-visible transition-[max-height,opacity] duration-[400ms] ease-in-out lg:flex lg:items-center lg:justify-between lg:w-full lg:max-h-none lg:opacity-100 lg:pointer-events-auto lg:py-0 lg:transition-none z-[1100] ${
            menuOpen
              ? "max-h-[25rem] opacity-100 pointer-events-auto py-[1.875rem]"
              : "max-h-0 opacity-0 pointer-events-none"
          }`}
        >
          <ul className="flex flex-col items-center gap-[0.9375rem] md:gap-5 lg:flex-row lg:gap-8 lg:mr-auto list-none">
            <li>
          ...
          <div className="flex flex-col w-full justify-center items-center gap-4 px-5 sm:px-8 mt-6 lg:mt-0 lg:flex-row lg:w-auto lg:items-center lg:px-0 lg:ml-auto">
            ...
            {/* Opciones exclusivas de escritorio */}
            <div className="hidden lg:flex lg:items-center lg:gap-[1.875rem]">
              ...
              {canAccessCart(user) && (
                <div className="relative" ref={cartWrapperRef}>
                  ...
                  {showCart && cartDropdown(() => setShowCart(false), false)}
                </div>
              )}
